...
install:
	mkdir -p $(DESTDIR)/usr/bin/ $(DESTDIR)/usr/lib/wb-mqtt-confed $(DESTDIR)/usr/share/wb-mqtt-confed/schemas $(DESTDIR)/var/lib/wb-mqtt-confed/schemas
	...
	install -m 0644 $(DEB_TARGET_ARCH).wbgo.so $(DESTDIR)/usr/lib/wb-mqtt-confed/wbgo.so
